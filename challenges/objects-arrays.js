...
const universities = graduates.map((state)=>{return state.university}).sort();
...
const contactInfo = graduates.map((state)=>{return `${state.first_name} ${state.email}` });
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */
const unisWithUni = graduates.filter((state)=>{return state.university.includes("Uni")});

// const unisWithUni = [];
// for(const x in graduates){
...
const lowCaseAnimalNames = zooAnimals.map((state)=>{return state.animal_name.toLowerCase()});
...
*/
const lowPopulationAnimals = zooAnimals.filter((state)=>{return state.population < 5});
...
*/
const populationTotal = zooAnimals.reduce((total, acc)=>{return total += acc.population},0);
